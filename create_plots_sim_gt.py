...
def plot_rewards_new(ground_truth, computed, title = "Ground Truth vs Computed Rewards", save_path = None, plot=False, method ='Computed Mean Reward' ):
    ...
    agents_fast = ["StochasticContinousFTGAgent_0.8_2_0.7_0.03_0.1_5.0_0.3_0.5", "StochasticContinousFTGAgent_0.6_2_0.8_0.03_0.1_5.0_0.3_0.5", "StochasticContinousFTGAgent_0.5_2_0.7_0.03_0.1_5.0_0.3_0.5"]
    agents_crash = ["pure_pursuit2_0.44_0.3_raceline1_0.3_0.5","pure_pursuit2_0.4_0.3_raceline4_0.3_0.5"]
    for agent in labels:
        if agent not in agents_fast and agent not in agents_crash:
            agent_index = labels.index(agent)
            sns.scatterplot(x= [gt_means[agent_index]], y = [computed_means[agent_index]], color='blue', s=100, alpha=0.7)
        #sns.scatterplot(x=gt_means, y=computed_means, color='blue', s=100, alpha=0.7)
    # plot the agent pure_pursuit2_0.44_0.3_raceline1_0.3_0.5 in red
    # extract with combined data where the agent is pure_pursuit2_0.44_0.3_raceline1_0.3_0.5
    
    for agent in agents_crash:
        agent_index = labels.index(agent)
        #print(agent_index)
        #print(gt_means[agent_index], computed_means[agent_index])
        sns.scatterplot(x= [gt_means[agent_index]], y = [computed_means[agent_index]], color='red', s=100, alpha=0.7)
    for agent in agents_fast:
        agent_index = labels.index(agent)
        #print(agent_index)
        #print(gt_means[agent_index], computed_means[agent_index])
        sns.scatterplot(x= [gt_means[agent_index]], y = [computed_means[agent_index]], color='orange', s=100, alpha=0.7)
...
def main(args):
    ...
    target_rewards_folder = f"/home/fabian/msc/f110_dope/ws_release/experiments/Sim/sim_{args.target_reward}"
    ...
    plot_rewards_new(ground_truth_rewards, compute_rewards, title=f"{reward_string}-reward\n rank-corr: {spearman_corr:.2f} (p={p_value:.3f}), mean-absolute-error: {abs:.1f}",#Reward: {args.target_reward}, {args.dynamics_model}, ",
                 save_path=f"plots_simulation/gt_sim{args.target_reward[:-5]}_spearman_corr.pdf", plot=args.plot, method="Simulation Estimate")
    ...
    plot_bars_from_dicts([ground_truth_rewards, compute_rewards], dict_names=["Ground Truth", "Computed"], add_title=f"Reward: {args.target_reward}, {args.dynamics_model} , Absolute Error: {abs:.2f},",#",
                         save_path=f"plots_simulation/gt_sim_{args.target_reward[:-5]}_bars.png", plot=args.plot)
    ...
    with open(f"plots_simulation/gt_sim_{args.target_reward[:-4]}_metrics.json", "w") as f:
