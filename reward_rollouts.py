...
def main(args):
    ...
    val_indices = np.load(os.path.join(save_path, "val_indices.npy"))
    ...
    for agent_name in np.unique(model_names):
        ...
        states_rollout, actions_rollout = model.rollout(inital_states,get_target_action= get_target_actions, horizon=horizon+1)
        ...
        reward = F110Env.compute_reward_trajectories(states_unnormalized,
                                        actions_rollout, 
        ...
        result_dict[agent_name] = {"mean": mean_reward_rollouts,
                                   "std": std_reward_rollouts}
        # print("Mean reward rollouts:", mean_reward_rollouts)
        #print("Mean reward gt:", mean_reward_gt)
        if args.compute_gt or args.plot:
            print("Mean reward rollouts:", mean_reward_rollouts)
        ...
        if args.plot:
            ...
            F110Env.plot_trajectories(states_unnormalized.squeeze().detach().numpy(),model_names[curr_trajectories], terminations_rollout, truncations_rollout, title = f"Rollouts {agent_name}")
            if args.compute_gt:
