...
def plot_validation_trajectories(behavior_dataset, model, F110Env, test_indices, num_plot, save_path=None, file_name=None):
    ...
    for start, end, model_name in zip(starts, ends,model_names):
        ...
        rollout_states, actions = model.rollout(states, actions, get_target_actions, horizon=states.shape[1])
        plt.plot(rollout_states[0,:end-start+1,0], rollout_states[0,:end-start+1,1], color="red")
        plt.plot(states[0,:end-start+1,0], states[0,:end-start+1,1], color="blue")
...
def compute_mse_rollouts(behavior_dataset, model, F110Env, test_indices):
...
def main(args):
    ...
    save_path = ope_methods.dataset.create_save_dir(
        experiment_directory = "runs2",
        algo=args.dynamics_model,
        reward_name="reward_progress",
        dataset="f110-real-stoch-v1",
    ...
    behavior_dataset = F110Dataset(
        ...
        remove_agents= [], # include all agents in the dataset
    )
    #print("dw")
    #print(behavior_dataset.states.shape)
    #print(behavior_dataset.actions.shape)
    # print(behavior_dataset.next_states.shape)
    # print(behavior_dataset.next_actions.shape)
    #print(behavior_dataset.rewards.shape)
    #print(behavior_dataset.masks.shape)
    #print(behavior_dataset.model_names.shape)
    #print(behavior_dataset.finished.shape)
    #print(behavior_dataset.finished.sum())
    #print(len(behavior_dataset))
    ...
    if args.split == "on-policy":
        ...
        #train_indices, test_indices, val_indices = trajectory_split_indices(
        #    behavior_dataset, behavior_dataset.trajectories)
    else:
    ...
    for i in pbar:
        ...
        if i % args.eval_interval == 0:
            ...
            val_mse = compute_mse_rollouts(behavior_dataset, model, F110Env, val_indices)
            writer.add_scalar(f"eval/mse_trajectory_25", np.mean(val_mse[:25].numpy()), global_step=i)
            writer.add_scalar(f"eval/mse_trajectory_50", np.mean(val_mse[:50].numpy()), global_step=i)
            writer.add_scalar(f"eval/mse_trajectory_max", np.max(val_mse.numpy()), global_step=i)
